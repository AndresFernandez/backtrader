...
## Este broker conserva la mayoría de caracteristicas del Backbroker sin embargo fué modificado para que funcione con velas de bid y ask,
## para ello se modifican las funciones de _try_exec_market,_try_exec_limit y _try_exec_stop, se agregan los precios de open,
#high y low del bid y ask y se sobreescribe el llamado a las funciones
...
class BaBroker(BrokerBase):
    ...
    def _try_exec_market(self, order, popena,popenb, phigha, plowb):
        ago = 0
        ...
        if order.isbuy():
            exprice = popena
            p = self._slip_up(phigha, exprice, doslip=self.p.slip_open)
        else:
            exprice = popenb
            p = self._slip_down(plowb, exprice, doslip=self.p.slip_open)

        self._execute(order, ago=0, price=p, dtcoc=dtcoc)

    def _try_exec_close(self, order, pclose,pclosea,pcloseb):
        ...
        if dt0 > order.created.dt:  # can only execute after creation time
            ...
            if dt0 >= order.dteos:
                ...
                if order.pannotated and dt0 > order.dteos:
                    ago = -1
                    if not order.isbuy():
                      execprice = order.pannotateda  
                    else:
                      execprice = order.pannotatedb
                else:
                    ago = 0
                    if  not order.isbuy():
                      execprice = pclosea  
                ...
                self._execute(order, ago=ago, price=20)
                return

        # If no exexcution has taken place ... annotate the closing price
        # Se agregan los utlimos precios del bid , ask y se conserva el standard
        order.pannotated = pclose
        order.pannotateda = pclosea
        order.pannotatedb = pcloseb


    def _try_exec_limit(self, order, popena, phigha, plowa,popenb, phighb, plowb, plimit):
        if order.isbuy():
            if plimit >= popena:
                # open smaller/equal than requested - buy cheaper
                pmax = min(phigha, plimit)
                p = self._slip_up(pmax, popena, doslip=self.p.slip_open,
                                  lim=True)
            ...
            if plimit <= popenb:
                ...
                pmin = max(plowb, plimit)
                p = self._slip_down(plimit, popenb, doslip=self.p.slip_open,
                                    lim=True)
                self._execute(order, ago=0, price=p)
            elif plimit <= phighb:
    ...
    def _try_exec_stop(self, order, popena, phigha, plowa,popenb, phighb, plowb, pcreated, pclose):
        if order.isbuy():
            if popena >= pcreated:
                # price penetrated with an open gap - use open
                p = self._slip_up(phigha, popena, doslip=self.p.slip_open)
                self._execute(order, ago=0, price=p)
            elif phigha >= pcreated:
                # price penetrated during the session - use trigger price
                p = self._slip_up(phigha, pcreated)
                self._execute(order, ago=0, price=p)

        else:  # Sell
            if popenb <= pcreated:
                # price penetrated with an open gap - use open
                p = self._slip_down(plowb, popenb, doslip=self.p.slip_open)
                self._execute(order, ago=0, price=p)
            elif plowb <= pcreated:
                # price penetrated during the session - use trigger price
                p = self._slip_down(plowb, pcreated)
                self._execute(order, ago=0, price=p)
    ...
    def _try_exec(self, order):
        ...
        ## Open original ,ask y bid
        popen = getattr(data, 'tick_open', None)
        if popen is None:
            popen = data.open[0]
        popena = getattr(data, 'tick_opena', None)
        if popena is None:
            popena = data.opena[0]
        popenb = getattr(data, 'tick_openb', None)
        if popenb is None:
            popenb = data.openb[0]

        ## High original ,ask y bid
        phigh = getattr(data, 'tick_high', None)
        if phigh is None:
            phigh = data.high[0]
        phigha = getattr(data, 'tick_higha', None)
        if phigha is None:
            phigha = data.higha[0]
        phighb = getattr(data, 'tick_highb', None)
        if phighb is None:
            phighb = data.highb[0]

        ## Low original ,ask y bid
        plow = getattr(data, 'tick_low', None)
        if plow is None:
            plow = data.low[0]
        plowa = getattr(data, 'tick_lowa', None)
        if plowa is None:
            plowa = data.lowa[0]
        plowb = getattr(data, 'tick_lowb', None)
        if plowb is None:
            plowb = data.lowb[0]

        ## Close original ,ask y bid        
        pclose = getattr(data, 'tick_close', None)
        if pclose is None:
            pclose = data.close[0]
        pclosea = getattr(data, 'tick_closea', None)
        if pclosea is None:
            pclosea = data.closea[0]
        pcloseb = getattr(data, 'tick_closeb', None)
        if pcloseb is None:
            pcloseb = data.closeb[0]
        ...
        if order.exectype == Order.Market:
            self._try_exec_market(order, popena,popenb, phigha, plowb)

        elif order.exectype == Order.Close:
            self._try_exec_close(order, pclose,pclosea,pcloseb)

        elif order.exectype == Order.Limit:
            self._try_exec_limit(order, popena, phigha, plowa,popenb, phighb, plowb, pcreated)
            ...
            self._try_exec_stop(order, popena, phigha, plowa,popenb, phighb, plowb, pcreated, pclose)
